...
class muninn.core.I18n
  ...
  constructor : ($config, $module = '') ->
    ...
    muninn.logMessage 'debug', "I18n Class Initialized"
  ...
  load: ($langfile, $module = @module, $code = '', $return = false) ->
    ...
    $langfile = $langfile.replace(muninn.EXT, '')+'.json'
    ...
    if not $found
      muninn.logMessage 'error', 'Unable to load the requested i18n file: i18n/' + $code + '/' + $langfile
      return

    if not $lang?
      muninn.logMessage 'error', 'I18n file contains no data: i18n/' + $code + '/' + $langfile
      return
    ...
    muninn.logMessage 'debug', 'I18n file loaded: i18n/%s/%s', $code, $langfile
    return true
  ...
  line : ($line = '') ->
    ...
    if $line is false
      muninn.logMessage('error', 'Could not find the i18n line "' + $line + '"')
