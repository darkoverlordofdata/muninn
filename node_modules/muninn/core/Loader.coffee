...
class muninn.core.Loader

  {array, rtrim, ltrim, ucfirst} = muninn.helpers # some util functions
  ...
  #
  # @param  [muninn.core.Object]  controller  the parent controller object
  #
  constructor: ($controller) ->
    ...
    @_model_paths = muninn.config.item('model_paths')
    @_view_ext = muninn.config.item('view_ext')
    @_view_paths = muninn.config.item('view_paths')
    @_class_paths = muninn.config.item('controller_paths')

    muninn.logMessage 'debug', "Loader Class Initialized"
  ...
  model: ($model, $name = '', $db_conn = false) ->
    ...
    if @controller[$name]?
      muninn.showError 'The model name you are loading is the name of a resource that is already being used: %s', $name

    for $path in muninn.config.getPaths(@controller.module, @_model_paths)

      if fs.existsSync($fullpath = $path+'models/'+$subdir+$model+muninn.EXT)

        if $db_conn isnt false and not muninn.db.DbDriver?
        ...
        Object.defineProperty @controller, $name,
          enumerable  : true
          writeable   : false
          value       : muninn.magic(@controller, require($fullpath), @controller)

        @_models.push $name
        return @controller[$name]

    # couldn't find the model
    muninn.showError 'Unable to locate the model you have specified: %s', $model
    return
  ...
  # @return [muninn.db.Driver]  the connected database driver
  #
  database: ($params = '', $return = false, $active_record = null) ->

    # Do we even need to load the database class?
    if muninn.db?
      if (muninn.db.DbDriver? and $return is false and $active_record is null and @controller['db']?)
        return false

    $db = @_db_factory($params || muninn.dbDriver, $active_record)
    ...
    try
      ...
      muninn.logMessage 'debug', '<--- defineProperty issue handled'
  ...
  # @param  [String]  params  either a resource url specifier or simple the driver name
  # @return	[Boolean]	return  whether to return the DB object
  # @return [muninn.db.Utility]  the connected database utility driver
  #
  dbutil: ($params = '', $return = false) ->

    if $params is ''
      if not muninn.db.DbDriver?
        @database()
      $db = @controller.db
    else
      $db = @database($params, true)

    require path.join(munnin.SYSPATH, 'db/Forge.coffee')
    require path.join(munnin.SYSPATH, 'db/Utility.coffee')
    $class = require(path.join(munnin.SYSPATH, 'db/drivers/', $db.dbdriver, '/', ucfirst($db.dbdriver), 'Utility.coffee'))

    if $return is true then return new $class(@controller, $db)
    Object.defineProperty @controller, 'dbutil',
      enumerable  : true
      writeable   : false
  ...
  # @param  [String]  params  either a resource url specifier or simple the driver name
  # @return	[Boolean]	return  whether to return the DB object
  # @return [muninn.db.Forge]  the connected database forge driver
  #
  dbforge: ($params = '', $return = false) ->
    ...
    require path.join(munnin.SYSPATH, 'db/Forge.coffee')
    $class = require(path.join(munnin.SYSPATH, 'db/drivers/', $db.dbdriver, '/', ucfirst($db.dbdriver), 'Forge.coffee'))

    if $return is true then return new $class(@controller, $db)
    Object.defineProperty @controller, 'dbforge',
      enumerable  : true
      writeable   : false
      value       : new $class(@controller, $db)
  ...
  # @return [Void]
  #
  view: ($res, $view, $vars = {}, $next = null) ->

    #
    # If we get an Error object, show it as content
    #
    if $view instanceof Error
      $vars = {err: new Error($view)}
      $view = path.join(munnin.APPPATH, 'errors/5xx')
    else if $vars instanceof Error
      $vars = {err: new Error($vars)}
      $view = path.join(munnin.APPPATH, 'errors/5xx')
    else if 'string' is typeof($view)

      #
      # Default extension: '.ect'
      #
      ...
      if $view.charAt(0) is '/'
        ...
        if $pos isnt -1
          ...
          if muninn.config.modules[$module]?
            $view = $view.substr($pos+1)
          else
            $pos = -1
        ...
        $path = false
        for $root in muninn.config.getPaths($module, @getViewPaths())
      ...
      if $path is false # then we didn't find it
        if fs.existsSync(muninn.APPPATH+'views/'+$orig+$ext)
          $view = $orig
          $path = muninn.APPPATH+'views/'
      ...
      if not fs.existsSync($path)
        return muninn.showError('Unable to load the requested file: %s', $file)
    ...
    @controller.render $res, $path, @_cached_vars, ($err, $html) =>
  ...
  helper: ($helpers) ->
    ...
    #
    $ext_helper = muninn.APPPATH+'helpers/'+muninn.config.item('subclass_prefix')+'_'+$helper+munnin.EXT
    if fs.existsSync($ext_helper)

      $base_helper = muninn.SYSPATH+'helpers/'+$helper+muninn.EXT
      if not fs.existsSync($base_helper)
        muninn.showError 'Unable to load the requested base file: helpers/%s', $helper+muninn.EXT
      ...
      muninn.logMessage 'debug', 'Helper loaded: '+$helper
      return @controller.server.setHelpers @_helpers[$helper]

    #
    # Search modules, then application
    #
    for $path in muninn.config.getPaths(@controller.module, @getClassPaths())
      if fs.existsSync($path+'helpers/'+$helper+muninn.EXT)
        @_helpers[$helper] = require($path+'helpers/'+$helper+muninn.EXT)
        # export?
        if @_helpers[$helper].is_global is true
          for $name, $body of @_helpers[$helper]
            define $name, $body
    ...
    muninn.showError 'Unable to load the requested file: helpers/%s', $helper+muninn.EXT
  ...
  config: ($file = '', $use_sections = false, $fail_gracefully = false) ->
    muninn.config.load($file, $use_sections, $fail_gracefully, @controller.module)
  ...
  driver: ($library = '', $params = null, $object_name = null) ->
    ...
    if not muninn.core.Driver?
      # we aren't instantiating an object here, that'll be done by the Library itself
      require muninn.SYSPATH+'lib/Driver.coffee'
  ...
  addModulePath: ($path) ->
    ...
    #
    muninn.config.paths.unshift $path if muninn.config.paths.indexOf($path) is -1
    return
  ...
  removeModulePath: ($path = '') ->
    ...
    muninn.config.paths.unshift APPPATH if muninn.config.paths.indexOf(APPPATH) is -1
  ...
  _load_class : ($class, $params = null, $object_name = null) ->
    $class = ltrim($class, '/').replace(muninn.EXT, '')
    $class = $class.replace(muninn.EXT, '')
    ...
    #
    $subclass = muninn.APPPATH + 'lib/' + $subdir + muninn.config.item('subclass_prefix') + $class + muninn.EXT
    if fs.existsSync($subclass)
      $baseclass = muninn.SYSPATH + 'lib/' + $class + muninn.EXT

      if not fs.existsSync($baseclass)
        muninn.logMessage('error', "Unable to load the requested class: %s", $class)
        muninn.showError("Unable to load the requested class: %s", $class)

      if @_loaded_files.indexOf($subclass) isnt -1
        if $object_name?
          if not @controller[$object_name]?
            return @_init_class(muninn.APPPATH, $class, muninn.config.item('subclass_prefix'), $params, $object_name)
    ...
    $is_duplicate = false
    for $path in muninn.config.getPaths(@controller.module, @getClassPaths())
      $filepath = path.join($path, 'lib/' + $subdir + $class + muninn.EXT)
    ...
    # Not found
    #
    return muninn.showError("Unable to load the requested class: %s", $class) unless $is_duplicate
    return
  ...
  _init_class : ($path, $class, $prefix = '', $params = {}, $object_name = null, $klass) ->
    ...
    $paths = if $path is muninn.APPPATH then [muninn.APPPATH] else [$path, muninn.APPPATH]
    for $path in $paths
      if fs.existsSync($path + 'config/' + $class.toLowerCase() + muninn.EXT)
        $config[$key] = $val for $key, $val of require($path + 'config/' + $class.toLowerCase() + muninn.EXT)

        if fs.existsSync($path + 'config/' + muninn.ENVIRONMENT + '/' + $class.toLowerCase() + muninn.EXT)
          $config[$key] = $val for $key, $val of require($path + 'config/' + muninn.ENVIRONMENT + '/' + $class.toLowerCase() + muninn.EXT)
        break
    ...
    Object.defineProperty $controller, $classvar,
      enumerable  : true
      writeable   : false
      value       : muninn.magic($controller, $klass, $controller, $config)
  ...
  initialize:  ->
    ...
    $autoload = {}
    if fs.existsSync($path = path.join(muninn.APPPATH, 'config/autoload.coffee'))
      $autoload[$key] = $val for $key, $val of require($path)

    if fs.existsSync($path = path.join(muninn.APPPATH, 'config/', muninn.ENVIRONMENT, '/autoload.coffee'))
      $autoload[$key] = $val for $key, $val of require($path)
  ...
  _db_factory: ($params = '', active_record_override = null) ->
    ...
    if 'string' is typeof($params)
      if $params.indexOf('://') is -1
        ...
        if not fs.existsSync($file_path = muninn.APPPATH + 'config/' + muninn.ENVIRONMENT + '/database.coffee')
          if not fs.existsSync($file_path = muninn.APPPATH + 'config/database.coffee')
            muninn.showError('The configuration file database.coffee does not exist.')

      {db, active_group, active_record} = require($file_path)

      if not db?  or Object.keys(db).length is 0
        muninn.showError 'No database connection settings were found in the database config file.'

      if $params isnt ''
        active_group = $params

      if not active_group? or not db[active_group]?
        muninn.showError 'Invalid database connection group.'
      ...
      if db[active_group]['url']?
        ...
        if ($dns = parse_url(db[active_group]['url'])) is false
          muninn.showError 'Invalid DB Connection String'
    ...
    if not $params['dbdriver']?  or $params['dbdriver'] is ''
      muninn.showError('DB Connection not specified.')

    if active_record_override isnt null
      active_record = active_record_override

    DbDriver = require(muninn.SYSPATH + 'db/Driver.coffee')
    if not active_record?  or active_record is true
      DbActiveRecord = require(muninn.SYSPATH + 'db/ActiveRecord.coffee')
      if not muninn.db.DbDriver?
        class muninn.db.DbDriver extends DbActiveRecord

    else if not muninn.db.DbDriver?
      class muninn.db.DbDriver extends DbDriver

    if not fs.existsSync(muninn.SYSPATH + 'db/drivers/' + $params['dbdriver'] + '/' + ucfirst($params['dbdriver']) + 'Driver.coffee')
      throw new Error("Unsuported DB driver: " + $params['dbdriver'])

    $driver = load_class(muninn.SYSPATH + 'db/drivers/' + $params['dbdriver'] + '/' + ucfirst($params['dbdriver']) + 'Driver.coffee')
...
module.exports = muninn.core.Loader