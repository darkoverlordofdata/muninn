...
class muninn.core.Connect
  ...
  constructor: () ->
    ...
    Object.defineProperties @, version  : {writeable: false, value: $version}
    @initialize($driver)
  ...
  initializeAssets:($driver, $render) ->
    ...
    #
    @app.use $driver.static(muninn.APPPATH+"assets/")
    @app.use $driver.static(muninn.DOCPATH) unless muninn.DOCPATH is false
    for $name, $module of @config.modules
      if muninn.isDir($module.path+"/assets/")
        @app.use $driver.static($module.path+"/assets/")
        muninn.logMessage 'debug', 'Module %s mounted at %s', $module.name, $module.path

    #
    # Favorites icon
    #
    @app.use $driver.favicon(muninn.APPPATH+"assets/" + @config.item('favicon'))
  ...
  initializeResponse: ($driver, $render) ->
    @app.use ($req, $res, $next) =>
      ...
      # Represent
      #
      $res.setHeader 'X-Powered-By', "muninn/#{@controller.version ? 4.2.0}"
      #
      # get the base url?
      #
...
module.exports = muninn.core.Connect
