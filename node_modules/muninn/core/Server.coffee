...
class muninn.core.Server extends muninn.core.Async
  ...
  initialize:($driver) ->
    ...
    $render = new Render()
    Response  = muninn.loadClass('Response', 'core')
    $response = Response
  ...
  start: ($router, $ready) ->
    ...
    for $name in @config.item('autoload').core
      Component = muninn.loadClass($name, 'core')
      @[$name] = new Component(@)

    # Load selected library components
    for $name in @config.item('autoload').libraries
      Component = muninn.loadClass($name, 'lib')
      @[$name] = new Component(@)
